...
#Prepare the data for the MaxEnt
...
head(fe.gbif); dim(fe.gbif); str(fe.gbif) # 103  24
fe.gbif.df <- fe.gbif[, (colnames(fe.gbif) %in% keep.dat)] # retrieve species records
head(fe.gbif.df); dim(fe.gbif.df) # 103  19
...
### Logistic
#error?
swd <- rbind(fe.gbif.df, sample.df.keep); dim(swd) 
